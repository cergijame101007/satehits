...
| 2 | スケジュール未設定の日付 | 単体 | デフォルト値（定休日ルールに従う）が返る |
| 3 | 残り食数の計算 | 単体 | 提供可能数 - 承認済み予約人数 |
...
| 2 | 存在しないメールアドレス | 単体 | `UNAUTHORIZED` エラー |
| 3 | パスワード不一致 | 単体 | `UNAUTHORIZED` エラー |
...
| 3 | ReservationForm | 定休日がカレンダーで選択不可 | 木・金曜が無効化されている |
