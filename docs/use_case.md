...
| UC-O14 | 予約を登録する | Instagram / 電話 / ウォークイン / その他の予約を登録する |
| UC-O15 | 予約をキャンセルする | 申請中・承認済みの予約をキャンセル（cancelled）にする |
...
| UC-S04 | pending 未対応リマインドメールを送信する | `pending` のまま来店 3日前・1日前にオーナーへ通知 | 実装しない（ADR-016） |
...
| 基本フロー | 1. 顧客が予約情報を入力する<br>2. システムがバリデーションを行う<br>3. Turnstileで認証する<br>4. 予約が登録される<br>5. 完了画面が表示される |
...
|------|------|
| アクター | オーナー、開発者 |
| 事前条件 | ログイン済み、対象予約が「申請中（pending）」または「承認済み（approved）」状態 |
| 事後条件 | 予約が「キャンセル（cancelled）」状態になる。予約済み人数（pending + approved）から外れ、残り食数が戻る |
| 基本フロー | 1. 予約一覧から対象予約を選択する<br>2. 「キャンセル」操作を選ぶ<br>3. ステータスが更新される（`PATCH /admin/reservations/{id}/status`、`status=cancelled`） |
| 備考 | 遷移は pending / approved → cancelled のみ。顧客へのメール送信はない（Outbox に enqueue するのは受付・承認・拒否のみ） |
...
| 備考 | enqueue は予約操作と同一 Tx（失敗は予約ごとロールバック）。送信失敗は Outbox が再送し、上限で `failed` |
...
| 送信元 | noreply@satehits.com |
| 備考 | enqueue は予約操作と同一 Tx（失敗は予約ごとロールバック）。送信失敗は Outbox が再送し、上限で `failed`。拒否理由は PATCH の任意 `reason` で受け取りメール本文にのみ反映（DB 非保存） |

### UC-S04: pending 未対応リマインドメールを送信する（不採用）

不採用（ADR-016。再検討トリガーは ADR 参照）。オーナーが新規予約に気づく一次手段は管理画面の pending 一覧とする。
