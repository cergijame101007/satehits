...
- 未対応 pending（今日以降）が 1 件以上あるとき、見出し直下に赤系の注意帯「未対応の予約が N 件あります。承認または拒否を行ってください」と「予約一覧へ」リンク（`/admin/reservations`）を表示する。クイックリンク「予約一覧」にも共通ナビと同じ赤バッジを付ける。0 件なら何も出さない
