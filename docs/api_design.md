...
指定日または指定月の残り食数を取得する。クエリは **単日モード**（`date`）と **月次モード**（`year` + `month`）のいずれか一方を指定する。両方を同時に指定した場合は **400 Bad Request**。
...
| visit_time | 11:30-13:30（日曜朝営業の朝 8:30〜は予約不可。店舗開店と予約受付開始は別） |
| phone | 電話番号形式 |
| email | メールアドレス形式 |
| note | 最大500文字 |
...
レスポンスヘッダに `Retry-After`（秒。残り時間の端数は切り上げ、最小 1）を付与する。待ち時間は `message` 本文にも含める。`Retry-After` は `Access-Control-Expose-Headers` で公開しているため、ブラウザの JS からも読める。
...
Instagram・電話・ウォークインなど、オーナーが手動で予約を登録する。Web 申請（`POST /reservations`）とは異なり、**`source` を必ず指定**し、**`status` を省略可能**（省略時は `approved`）。
